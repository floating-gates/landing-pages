<script setup>
import { themeColor, themeColorOrange, themeColorWhite } from "../config.js";
import mainPhoto from "../data/images/platform_logic_pic.webp";
import Header from "../components/Header.vue";
import background from "../data/images/background_pic.webp";
import { PriceEstimationIcon, CircleCheckmarkIcon, BarChartSquareIcon } from "../helper/icons_generator.js";

const missionSubHeading = "Scaling Automated Manufacturing"

const features = [
    { 
        title: 'Factory-wide Manufacturability Checks', 
        description: 'Verify manufacturing feasibility in real time with robust automated analysis and warn users of modifications' 
    },
    { 
        title: 'Conversation on CAD', 
        description: 'Follow what you customer is asking at an unprecedented precision, with unmistakable precision.' 
    },
    { 
        title: 'Immediate Price Estimation', 
        description: 'Provide to your customers fast and transparent quotes aligned with current market standards. Parsing bad prospects at the root.' 
    },
    // { 
    //     title: 'Device Optimization', 
    //     description: 'Enhance device performance with optimization tools to make manufacturing easier and improving the customer satisfaction' 
    // }
]
</script>

<template>
<Header :context="'landing-page'" />
<div class="mission-hero-wrapper"
     :style="{ backgroundImage: `url(${background})`}">
  <div class="container mission-container">
    <div class="row justify-content-between align-items-center">
      <div class="col-lg-5">
        <div class="mission-content">
          <span
            class="section-badge"
            data-aos="fade-up"
            data-aos-delay="100" >
            NEVER MISS AN ERROR IN YOUR CAD ANYMORE
          </span>
          
          <h1 class="mission-title" data-aos="fade-up" data-aos-delay="100">
            {{ missionSubHeading }}
          </h1>
          <div class="mission-description" data-aos="fade-up" data-aos-delay="200">
            <p class="lead-text">
              We recognize the messy world we live in and the amount of files we collect in our library thinking they are manufacturable. Our mission is to maintain and monitor every file in your network to be production-ready, permitting to untap potential of your machines, to meet customizable production needs and target 100% usage span.
            </p>
          </div>
        </div>
      </div>
      
      <div class="col-lg-7" data-aos="fade-up" data-aos-delay="400">
        <div class="video-container">
          <img :src="mainPhoto" alt="Manufacturing Hub" class="hero-image" />
        </div>
      </div>
    </div>
  </div>  
</div>

<!-- How to Achieve Section -->
<section class="achievement-section bg-light flex justify-center items-center w-full">
  <div class="flex flex-col md:flex-row w-full max-w-[1600px] px-[5%] py-12 items-center justify-center gap-14">
    <div class="w-full md:w-1/2" data-aos="fade-up" data-aos-delay="100">
      <div class="strategy-content">
        <h3 class="heading">Make <em>Everyone</em> aware of what your Factory can do.</h3>
        <p class="strategy-text">
          We provide a B2B2C platform for factories with manufacturing capacity, enabling them to maximize their resources while serving both customers and engineers with unprecedented efficiency.
        </p>
        
        <div class="cta-section">
          <a href="/faq" class="cta-button" :style="{ backgroundColor: themeColor, borderColor: themeColor }">
            Learn More About Our Platform
          </a>
        </div>
      </div>
    </div>
    
    <div class="w-full md:w-1/2">
      <div class="features-grid">
        <div
          class="feature-card"
          v-for="(feature, index) in features"
          :key="index"
          :data-aos-delay="150 + index * 50" >
          <div class="feature-icon">
            <PriceEstimationIcon
              v-if="index === 2"
              width="32"
              height="32"
              :stroke="themeColorOrange"
            />
            <CircleCheckmarkIcon
              v-else-if="index === 1"
              width="32"
              height="32"
              :stroke="themeColorOrange"
            />
            <BarChartSquareIcon
              v-else-if="index === 0"
              width="32"
              height="32"
              :stroke="themeColorOrange"
            />
          </div>
          <div class="feature-content">
            <h4 class="feature-title">{{ feature.title }}</h4>
            <p class="feature-description">{{ feature.description }}</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
</template>

<style scoped>
.mission-hero-wrapper {
    width: 100%;
    background-size: cover;
    background-position: center;
    background-repeat: no-repeat;
    padding: 200px 0 120px;
}

.mission-container {
    /* padding: 200px 0 120px; */
    max-width: 1600px;
    padding-left: 5%;
    padding-right: 5%;
    margin: 0 auto;
}

.mission-title {
    font-size: 3.3rem;
    font-weight: 650;
    line-height: 1.2;
    margin-bottom: 30px;
    text-shadow: 0 1px 2px rgba(0,0,0,0.2);
    color: v-bind("themeColor");
}

.lead-text {
    font-size: 1.2rem;
    line-height: 1.7;
    margin-bottom: 40px;
    font-weight: 400;
}

.mission-points {
    display: flex;
    flex-direction: column;
    gap: 20px;
}

.point-item {
    display: flex;
    align-items: center;
    padding: 20px;
    background: white;
    border-radius: 16px;
    box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);
    transition: all 0.3s ease;
    border-left: 4px solid v-bind("themeColorOrange");
}

.point-item:hover {
    transform: translateX(8px);
    box-shadow: 0 8px 30px rgba(0, 0, 0, 0.15);
}

.point-icon {
    font-size: 2rem;
    margin-right: 20px;
    flex-shrink: 0;
}

.point-text {
    font-size: 1.1rem;
    font-weight: 500;
    color: #333;
}

/* Video Section */
.video-container {
    position: relative;
    border-radius: 14px;
    overflow: hidden;
    box-shadow: 0 20px 60px rgba(0, 0, 0, 0.15);
}

.hero-image {
    width: 100%;
    height: auto;
    display: block;
    transition: transform 0.2s;
}

.video-overlay {
    position: absolute;
    display: flex;
    align-items: center;
    justify-content: center;
    transition: all 0.3s ease;
    z-index: 3;
}

.video-overlay:hover .video-container .hero-image {
    transform: scale(1.05);
}

/* Achievement Section */
.achievement-section {
    padding: 30px 0;
    position: relative;
}

.section-header {
    margin-bottom: 60px;
}

.section-badge {
    font-size: 1.4rem;
    font-weight: 350;
    color: v-bind(themeColorOrange);
    margin-bottom: 16px;
}

.section-title {
    font-size: 2.4rem;
    font-weight: 600;
    color: #2c3e50;
    margin-bottom: 20px;
}

.title-underline {
    width: 65%;
    height: 4px;
    margin: 0 auto;
    border-radius: 2px;
}

.strategy-content {
    padding-right: 40px;
}

.strategy-heading {
    font-size: 2.2rem;
    font-weight: 600;
    color: #2c3e50;
    margin-bottom: 24px;
}

.strategy-text {
    font-size: 1.1rem;
    line-height: 1.8;
    color: #666;
    margin-bottom: 40px;
}

.cta-button {
    display: inline-flex;
    align-items: center;
    padding: 16px 32px;
    border: none;
    border-radius: 50px;
    color: white;
    font-weight: 600;
    font-size: 1rem;
    text-decoration: none;
    transition: all 0.3s ease;
    box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15);
}

.cta-button:hover {
    transform: translateY(-2px);
    box-shadow: 0 8px 30px rgba(0, 0, 0, 0.25);
}

.features-grid {
    display: flex;
    flex-direction: column;
    gap: 24px;
}

.feature-card {
    display: flex;
    align-items: flex-start;
    padding: 32px;
    background: white;
    border-radius: 20px;
    box-shadow: 0 8px 30px rgba(0, 0, 0, 0.08);
    transition: all 0.3s ease;
    border: 1px solid #f0f0f0;
}

.feature-card:hover {
    transform: translateY(-8px);
    box-shadow: 0 16px 50px rgba(0, 0, 0, 0.15);
}

.feature-icon {
    width: 60px;
    height: 60px;
    border-radius: 16px;
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 1.8rem;
    margin-right: 24px;
    flex-shrink: 0;
}

.feature-title {
    font-size: 1.3rem;
    font-weight: 600;
    color: #2c3e50;
    margin-bottom: 8px;
}

.feature-description {
    color: #666;
    line-height: 1.6;
    margin: 0;
}

/* Responsive Design */
@media (max-width: 992px) {
    .mission-title {
        font-size: 2.8rem;
    }
    
    .strategy-content {
        padding-right: 0;
        margin-bottom: 40px;
    }
    
    .point-item {
        padding: 16px;
    }
    
    .point-icon {
        font-size: 1.5rem;
        margin-right: 16px;
    }
}

@media (max-width: 768px) {
    
    .mission-title {
        font-size: 2.2rem;
    }
    
    .section-title {
        font-size: 2.2rem;
    }
    
    .strategy-heading {
        font-size: 1.8rem;
    }
    
    .achievement-section {
        padding: 80px 0;
    }
    
    .play-button {
    width: 60px;
    height: 60px;
  }
  
  .video-pulse {
    width: 100px;
    height: 100px;
  }
}
</style>
